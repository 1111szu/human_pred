...
class AttModelPerParts(Module):
    ...
    def forward(self, src, output_n=25, input_n=50, itera=1):
        ...
        for ii, pidx in enumerate(self.parts_idx):
            key_tmp.append(self.convK[ii](
                src_key_tmp[:, pidx] / 1000.0).unsqueeze(1))
        key_tmp = torch.cat(key_tmp, dim=1)

        for i in range(itera):
            query_tmp = []
            for ii, pidx in enumerate(self.parts_idx):
                query_tmp.append(self.convQ[ii](
                    src_query_tmp[:, pidx] / 1000.0).unsqueeze(1))
            query_tmp = torch.cat(query_tmp, dim=1)
            score_tmp = torch.matmul(
                query_tmp.transpose(2, 3), key_tmp) + 1e-15
            att_tmp = score_tmp / (torch.sum(score_tmp, dim=3, keepdim=True))
            dct_att_tmp = torch.zeros(
                [bs, self.in_features, self.dct_n]).float().cuda()
            for ii, pidx in enumerate(self.parts_idx):
