...
def main(opt):
    ...
    net_pred.cuda() if net_pred.cuda() else net_pred.cpu()
    ...
    ret_test = run_model(net_pred, is_train=2,
                         data_loader=test_loader, opt=opt, epo=0)
...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    idx = np.expand_dims(np.arange(seq_in + 1), axis=1) + \
        np.expand_dims(np.arange(out_n), axis=0)
    st = time.time()
    for i, (p3d_h36) in enumerate(data_loader):
        batch_size, seq_n, _, _ = p3d_h36.shape
        n += batch_size
        bt = time.time()
        p3d_h36.cuda() if net_pred.cuda() else p3d_h36.cpu()
        p3d_h36 = p3d_h36.float().cuda()[:, :, joint_used] * 1000
        p3d_sup = p3d_h36.clone()[:, -out_n - seq_in:]
        p3d_src = p3d_h36.clone().reshape(
            [batch_size, in_n + out_n, len(joint_used) * 3])
